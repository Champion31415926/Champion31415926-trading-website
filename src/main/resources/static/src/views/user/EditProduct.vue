<template>
  <div class="product-publish-container">
    <h1 class="publish-title">Edit Product</h1>
...
export default {
  ...
  computed: {
    oldLevelText() {
      if (this.product.oldLevel === 10) return "100% New";
      return `${this.product.oldLevel}0% New`;
    },
  },
  created() {
  ...
  methods: {
    getStorageProductInfo() {
      ...
      if (!this.product.oldLevel) this.product.oldLevel = 9;
    ...
    async editProduct() {
      ...
      this.editing = true;
      this.product.coverList = this.coverList
    ...
    async fetchCategoryList() {
      try {
        ...
        if (res.data.code === 200) {
          ...
          if (this.product.categoryId) {
            const currentCategory = this.categoryList.find(
        ...
        console.error("Error querying product category information:", error);
      }
    },
